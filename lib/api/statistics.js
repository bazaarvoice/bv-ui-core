...
// Polyfill for fetch that can be removed once browsers adopt.
...
function getStatistics (options) {
  ...
  const envPrefix = envPrefixMap[options.environment]

  const filters = Object.keys(options.filters)
    .map(filter => `&filter=${filter}:${options.filters[filter]}`)
    .join()
