 ...
 * @param {String} [sameSite] The SameSite attribute ('Strict', 'Lax', or 'None'). Defaults to 'Lax'.
 */
function createCookie (name, value, days, domain, secure, sameSite) {
  var date = new Date();
  ...
  // Determine SameSite attribute. Default to 'Lax' for CSRF protection.
  // Note: SameSite=None requires the Secure flag, so we fallback to 'Lax' if not secure.
  var sameSiteValue = sameSite || 'Lax';
